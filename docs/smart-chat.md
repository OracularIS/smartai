...
- **Cross-system workflows**: Ask once and let SmartForge coordinate multiple approved steps.
...
## Why SmartForge Cortex is different from “document chat”

SmartForge Cortex is designed for operations and execution. Instead of only searching documents (PDFs, wikis, emails), it focuses on **structured, actionable integrations** (approved APIs/commands/workflows). That means you can get live, up-to-date results—and perform approved actions—without needing to know the underlying system details.

<img src=".attachments/approach.png" alt="SmartForge Cortex Approach Diagram">
...
After SmartForge returns a table or dataset, you can refine it:
...
You can ask SmartForge to turn the latest dataset into an interactive dashboard.
SmartForge identifies key fields and data types and generates charts/metrics to highlight trends.

<img src=".attachments/auto_dashboard.png" alt="Auto-generated Dashboard Example" width="750">

Dashboards are created automatically without any manual configuration, with AI-powered intelligent chart type selection based on your data patterns. You get:
...
SmartForge Cortex doesn't just answer questions — you can create repeatable automated workflows directly from your chat conversation:
...
When you ask something in SmartForge Cortex, SmartForge typically:
...
For questions that span multiple systems, SmartForge can query systems in parallel and merge results into a single answer (so you don’t have to check multiple screens).

## Transparency

When enabled, SmartForge will show you **exactly what code was generated and executed** for your request:
...
<img src=".attachments/security_architecture.png" alt="SmartForge Cortex Security Architecture" width="750">

- SmartForge runs **approved functions only**. If something is not approved, it won’t run. No arbitrary code execution.
...
- For analytics (charts/dashboards), SmartForge uses a controlled approach: it only sends column names and small data samples to understand how to summarize results, while keeping full datasets 100% protected inside your environment.
